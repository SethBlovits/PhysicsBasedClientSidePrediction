...
public class ClientPrediction : NetworkBehaviour
{
    ...
    void Update()
    {
        if(IsOwner){
            ...
            while(timer>=minTimeBetweenTicks){
                ...
                simulateMovementTick(inputPacketCache[cacheIndex].horizontal,inputPacketCache[cacheIndex].vertical,inputPacketCache[cacheIndex].currentTick);
                if(!IsServer){
    ...
    void simulateMovementTick(float horizontal, float vertical,int currentTick){
        ...
        Vector3 movementVector = horizontal*m_rigidBody.transform.right+vertical*m_rigidBody.transform.forward;
        m_rigidBody.AddForce(movementVector.normalized*moveSpeed,ForceMode.Force);
    ...
    void sendClientInputsServerRpc(InputPacket[] inputPackets,RpcParams rpcParams = default){
        ...
        for(int i = 0;i<numInputPackets;i++){
            if(inputPackets[i].currentTick>lastSentTick){
                ...
                Debug.Log(tick);
                simulateMovementTick(horizontal,vertical,tick);
    ...
    void Reconciliation(){
        ...
        if(positionError > 0.0001f){ //|| rotationError > 0.0001f
            ...
            m_rigidBody.isKinematic = false;
            m_rigidBody.position = Vector3.Lerp(currentServerState.position,m_rigidBody.position,0.1f);//gonna need to lerp quaternions as well soon
            ...
            for(int i = correctionTick;i<clientTick;i++){
                ...
                simulateMovementTick(resimulatedPacket.horizontal,resimulatedPacket.vertical,resimulatedPacket.currentTick);
            }
